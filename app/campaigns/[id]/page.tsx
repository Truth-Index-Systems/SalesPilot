...
export default async function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Historical truth matters more than the current review queue. A valid batch
  ...
  return <AppShell title={campaign.name} user={user} workspaceStats={{ campaigns: campaignCount, companies: companyCount, replies: 0, opportunities: opportunityCount }}>
    ...
    <div className="hero campaign-control-centre">
      <div className="campaign-control-copy">
        ...
        <h2>{campaignPaused ? "This outbound sales campaign is paused." : opportunityResearchComplete > 0 ? `${opportunityResearchComplete} sales opportunit${opportunityResearchComplete === 1 ? "y is" : "ies are"} assembled.` : (contactsActive || opportunityBuilding > 0) ? "SalesPilot is building complete commercial opportunities." : routeHandoffReady ? "Company discovery is moving into route research." : discoveryExhausted ? "The extended company search is complete." : discoveryComplete ? "Company discovery is ready to progress." : discoveryRunning ? "SalesPilot is finding strong commercial matches." : discoveryPreparingFirstPass ? "SalesPilot is preparing company discovery." : discoveryPreparationRetry ? "Company discovery preparation will resume." : discoveryRetryScheduled ? "Company research retry is scheduled." : blockingDiscoveryNeedsAttention ? "Company discovery needs attention." : discoveryExpansionPending ? "SalesPilot is expanding the company search." : discoveryQueued ? "Company discovery is queued." : "Your outbound sales campaign is ready."}</h2>
        <p>{campaignPaused ? "Autonomous work is stopped. Your campaign and all supporting intelligence remain safely saved." : opportunityResearchComplete > 0 ? `${opportunityRecommended} recommended · ${opportunityAwaitingReview} awaiting your decision · ${opportunityApproved} approved for engagement. SalesPilot has combined company fit, commercial need, route quality and evidence into one ranked commercial view.` : (contactsActive || opportunityBuilding > 0) ? `${contactResearching} compan${contactResearching === 1 ? "y is" : "ies are"} being researched while SalesPilot connects commercial need, route quality and the reason to buy.` : routeHandoffReady ? `${approvedCompanyCount} approved companies are now being converted into complete opportunities.` : discoveryExhausted ? "SalesPilot completed every safe expansion pass without adding weak recommendations. Review or broaden the campaign strategy to continue." : discoveryComplete ? `${companyCount} compan${companyCount === 1 ? "y has" : "ies have"} been retained in supporting intelligence. SalesPilot will continue building the strongest opportunities.` : discoveryRunning ? "SalesPilot is continuing the approved campaign automatically. Low-confidence matches remain visible with transparent limitations." : discoveryPreparingFirstPass ? "SalesPilot is finalising the approved business and campaign context before the first research pass begins. No retry has occurred." : discoveryPreparationRetry ? "SalesPilot has not completed a company search pass yet. Preparation will resume automatically at the saved time." : discoveryRetryScheduled ? "A technical issue interrupted company research. SalesPilot will retry automatically at the saved retry time." : blockingDiscoveryNeedsAttention ? "The initial company search reached a terminal failure before SalesPilot established a usable company batch." : discoveryExpansionPending ? "The latest pass retained too few evidence-backed matches, so SalesPilot is automatically exploring another commercial angle. The evidence standard remains unchanged." : discoveryQueued ? "SalesPilot has accepted this campaign and will begin research as soon as the campaign context is ready." : "Business understanding and campaign approval are complete. SalesPilot is preparing commercial opportunities."}</p>
        <div className="campaign-readiness"><span/><strong>{opportunityResearchComplete > 0 ? "Opportunities" : (contactsActive || opportunityBuilding > 0) ? "Assembling opportunities" : routeHandoffReady ? "Route research next" : stageLabel}</strong><small>{opportunityResearchComplete > 0 ? `${opportunityAwaitingReview} awaiting decision · ${opportunityApproved} approved` : (contactsActive || opportunityBuilding > 0) ? `${contactResearching} researching · ${contactQueued} queued · ${contactRetryScheduled} retry scheduled` : routeHandoffReady ? "Approved companies are moving forward automatically" : discoveryExhausted ? `${companyCount} supported matches retained` : discoveryComplete ? `${companyCount} companies retained in intelligence` : discoveryPreparingFirstPass ? "Preparing first research pass" : discoveryPreparationRetry ? "Preparation retry scheduled" : discoveryRetryScheduled ? "Technical retry scheduled" : blockingDiscoveryNeedsAttention ? "Needs attention" : discoveryExpansionPending ? "Another evidence-backed pass is queued" : discoveryQueued ? "Queued" : progress !== null ? `${progress}% complete` : stageLabel}</small></div>
    ...
    <div className="grid cols-2 section campaign-primary-grid">
      ...
      <Card className="next-milestone-card">
        ...
        <div className="card-subtitle">{routeHandoffReady ? "Approved companies are progressing into buyer, reachability and commercial-route research." : reviewComplete ? "Every verified recommendation has received a workspace decision." : discoveryExhausted ? "SalesPilot preserved the evidence standard. Broaden the approved audience or commercial angle before continuing." : discoveryReadyForReview ? "Review the evidence-backed recommendations and approve the strongest commercial matches." : discoveryExpansionPending ? "SalesPilot is automatically exploring another supported commercial angle." : "SalesPilot has started this stage automatically from your approved campaign."}</div>
