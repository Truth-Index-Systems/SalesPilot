  ...
  return <AppShell title={campaign.name} user={user} workspaceStats={{ campaigns: campaignCount, companies: companyCount, replies: 0, opportunities: opportunityCount }}>
    ...
    <div className="hero campaign-control-centre">
      <div className="campaign-control-copy">
        ...
        <h2>{campaignPaused ? "This outbound sales campaign is paused." : opportunityCount > 0 ? `${opportunityCount} sales opportunit${opportunityCount === 1 ? "y is" : "ies are"} assembled.` : contactsActive ? "SalesPilot is building complete commercial opportunities." : reviewComplete ? "Company discovery is moving into route research." : discoveryExhausted ? "The extended company search is complete." : discoveryComplete ? "Company discovery is ready to progress." : discoveryRunning ? "SalesPilot is finding strong commercial matches." : discoveryPreparingFirstPass ? "SalesPilot is preparing company discovery." : discoveryPreparationRetry ? "Company discovery preparation will resume." : discoveryRetryScheduled ? "Company research retry is scheduled." : discoveryNeedsAttention ? "Company discovery needs attention." : discoveryExpansionPending ? "SalesPilot is expanding the company search." : discoveryQueued ? "Company discovery is queued." : "Your outbound sales campaign is ready."}</h2>
        <p>{campaignPaused ? "Autonomous work is stopped. Your campaign and all supporting intelligence remain safely saved." : opportunityCount > 0 ? `${opportunityRecommended} recommended · ${opportunityAwaitingReview} awaiting your decision · ${opportunityApproved} approved for engagement. SalesPilot has combined company fit, commercial need, route quality and evidence into one ranked commercial view.` : contactsActive ? `${contactResearching} compan${contactResearching === 1 ? "y is" : "ies are"} being researched while SalesPilot connects commercial need, route quality and the reason to buy.` : reviewComplete ? `${approvedCompanyCount} approved companies are now being converted into complete opportunities.` : discoveryExhausted ? "SalesPilot completed every safe expansion pass without adding weak recommendations. Review or broaden the campaign strategy to continue." : discoveryComplete ? `${companyCount} compan${companyCount === 1 ? "y has" : "ies have"} been retained in supporting intelligence. SalesPilot will continue building the strongest opportunities.` : discoveryRunning ? "SalesPilot is continuing the approved campaign automatically. Low-confidence matches remain visible with transparent limitations." : discoveryPreparingFirstPass ? "SalesPilot is finalising the approved business and campaign context before the first research pass begins. No retry has occurred." : discoveryPreparationRetry ? "SalesPilot has not completed a company search pass yet. Preparation will resume automatically at the saved time." : discoveryRetryScheduled ? "A technical issue interrupted company research. The scheduler will retry automatically at the saved retry time." : discoveryNeedsAttention ? "The job reached a terminal failure and requires an administrator to review the diagnostic reason." : discoveryExpansionPending ? "The latest pass retained too few evidence-backed matches, so SalesPilot is automatically exploring another commercial angle. The evidence standard remains unchanged." : discoveryQueued ? "The scheduler has accepted this campaign and will begin intelligence gathering when a worker is available." : "Business understanding and campaign approval are complete. SalesPilot is preparing commercial opportunities."}</p>
        <div className="campaign-readiness"><span/><strong>{opportunityCount > 0 ? "Opportunities" : contactsActive ? "Assembling opportunities" : reviewComplete ? "Buyer intelligence next" : stageLabel}</strong><small>{opportunityCount > 0 ? `${opportunityAwaitingReview} awaiting decision · ${opportunityApproved} approved` : contactsActive ? `${contactResearching} researching · ${contactQueued} queued · ${contactRetryScheduled} retry scheduled` : reviewComplete ? "Approved companies are moving forward automatically" : discoveryExhausted ? `${companyCount} supported matches retained` : discoveryComplete ? `${companyCount} companies retained in intelligence` : discoveryPreparingFirstPass ? "Preparing first research pass" : discoveryPreparationRetry ? "Preparation retry scheduled" : discoveryRetryScheduled ? "Technical retry scheduled" : discoveryNeedsAttention ? "Needs attention" : discoveryExpansionPending ? "Another evidence-backed pass is queued" : discoveryQueued ? "Queued" : progress !== null ? `${progress}% complete` : stageLabel}</small></div>
    ...
    <div className="grid cols-2 section campaign-primary-grid">
      ...
      <Card className="next-milestone-card">
        ...
        <div className="card-title large">{reviewComplete ? "Review complete" : discoveryExhausted ? "Review campaign strategy" : discoveryReadyForReview ? "Company Review" : discoveryExpansionPending ? "Search expanding" : "Company Discovery"}</div>
        <div className="card-subtitle">{reviewComplete ? "Every verified recommendation has received a workspace decision." : discoveryExhausted ? "SalesPilot preserved the evidence standard. Broaden the approved audience or commercial angle before continuing." : discoveryReadyForReview ? "Review the evidence-backed recommendations and approve the strongest commercial matches." : discoveryExpansionPending ? "SalesPilot is automatically exploring another supported commercial angle." : "SalesPilot has started this stage automatically from your approved campaign."}</div>
        ...
        <div className="milestone-state"><span className="roadmap-pulse"/><div><strong>{reviewComplete ? "Review decisions saved" : discoveryExhausted ? "Strategy adjustment available" : discoveryReadyForReview ? "Recommendations ready" : stageLabel}</strong><small>{reviewComplete ? `${approvedCompanyCount} approved · ${rejectedCompanyCount} not selected.` : discoveryExhausted ? `${companyCount} supported matches retained after the extended search.` : discoveryReadyForReview ? `${pendingCompanyCount} companies are waiting for review.` : progress !== null ? `${progress}% complete · progress is saved automatically.` : stageLabel}</small></div></div>
      </Card>
    </div>
