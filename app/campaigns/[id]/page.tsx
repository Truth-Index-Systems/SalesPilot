...
export default async function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const hasUsableCompanyProgress = pendingCompanyCount > 0 || approvedCompanyCount > 0;
  const blockingDiscoveryNeedsAttention = discoveryNeedsAttention && !hasUsableCompanyProgress;
  const backgroundDiscoveryNeedsAttention = discoveryNeedsAttention && hasUsableCompanyProgress;
  ...
  return <AppShell title={campaign.name} user={user} workspaceStats={{ campaigns: campaignCount, companies: companyCount, replies: 0, opportunities: opportunityCount }}>
    ...
    <div className="hero campaign-control-centre">
      ...
      <div className="next-status-panel">
        ...
        <strong>{opportunityApproved > 0 ? "Engagement" : opportunityCount > 0 ? "Opportunity Review" : (contactsActive || routeHandoffReady) ? "Route Research" : "Company Discovery"}</strong>
        <small>{opportunityCount > 0 ? `${opportunityRecommended} recommended · top score ${topOpportunity?.opportunity_score ?? 0}/100` : contactsActive ? `${contactResearching} companies researching · ${pendingContactCount} contacts awaiting review` : routeHandoffReady ? "Building buyer and reachability intelligence" : stageLabel}</small>
