...
export default async function Contacts({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <AppShell title="Contacts" user={user} workspaceStats={{ campaigns: campaigns.length, companies: companies.total, replies: 0, opportunities: 0 }}>
    <ContactAutoRefresh active={activeSessions.length > 0 || retrySessions.length > 0}/><PageHeader eyebrow="Ways into your target companies" title="Routes" subtitle="MarketRoute finds the people and practical contact paths most likely to get your message seen, with the evidence behind every recommendation." />
    ...
    <div className="contact-intelligence-grid">
      ...
      <Card className="contact-activity-card">
        <div className="section-head"><div><div className="card-title">MarketRoute activity</div><div className="card-subtitle">The latest customer-route research happening in the background.</div></div><span className={`live-dot ${researching ? "active" : ""}`}/></div>
        <div className="contact-activity-feed section">
          {latestActivity.length ? latestActivity.map(item => <div key={item.id} className="contact-activity-item"><span className={jobTone(item) === "complete" ? "complete" : jobTone(item) === "attention" ? "attention" : "active"}>{isJobComplete(item) ? <CheckCircle2 size={14}/> : <Activity size={14}/>}</span><div><strong>{companyById.get(item.company_id) ?? "Approved company"}</strong><small>{sessionLabel(item)}{item.contacts_saved ? ` · ${item.contacts_saved} saved` : ""}</small></div></div>) : <div className="contact-activity-empty"><Search size={18}/><span>Approve a company and MarketRoute will begin decision-maker research automatically.</span></div>}
        </div>
      </Card>
    </div>
    ...
    {rows.length ? <ContactReviewQueue rows={rows}/> : <Card><div className="empty"><h3>{hasFilters ? "No contacts match these filters" : researching ? "MarketRoute is researching decision-makers" : "No contacts yet"}</h3><p>{hasFilters ? "Adjust or clear the filters." : researching ? "Verified contacts will appear here as research completes." : "Approve companies to begin route research."}</p></div></Card>}

    <Card className="contact-next-stage"><div><span className="eyebrow">What happens next</span><h3>Personalised outreach</h3><p>Approved routes move into personalised outreach built around the reason this company should care.</p></div><div className="next-stage-count"><strong>{reachableApproved}</strong><span>approved and reachable</span></div><ArrowRight size={22}/></Card>
  </AppShell>;
}
