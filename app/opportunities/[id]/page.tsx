...
export default async function OpportunityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const boundaryCompleteness = opportunity.r4_boundary_completeness_json && typeof opportunity.r4_boundary_completeness_json === "object"
    ? opportunity.r4_boundary_completeness_json as Record<string, unknown> : {};
  const unresolvedBoundaryKeys = Array.isArray(boundaryCompleteness.unresolvedBoundaryKeys) ? boundaryCompleteness.unresolvedBoundaryKeys.map(String) : [];
  const missingBoundaryKeys = Array.isArray(boundaryCompleteness.missingMandatoryBoundaryKeys) ? boundaryCompleteness.missingMandatoryBoundaryKeys.map(String) : [];
  ...
  return <AppShell title={opportunity.company_name} user={user} workspaceStats={{ campaigns: campaigns.length, companies: new Set(all.map(row => row.company_id)).size, replies: 0, opportunities: all.length }}>
    ...
    <Card className="section">
      ...
      <div className="strategy-grid section">
        ...
        <div className="strategy-item"><CheckCircle2 size={18}/><div><span>R4 · Boundary constitution</span><strong>{opportunity.r4_boundary_complete ? "COMPLETE" : opportunity.r4_disposition === "RESEARCH_REQUIRED" ? "RESEARCH REQUIRED" : "INCOMPLETE"}</strong><small>{opportunity.r4_boundary_constitution_version || "No current constitution"}{[...unresolvedBoundaryKeys,...missingBoundaryKeys].length ? ` · ${[...unresolvedBoundaryKeys,...missingBoundaryKeys].join(", ")}` : " · all mandatory R4 boundaries represented"}</small></div></div>
      ...
      <div className="route-signal-grid section">
        ...
        <div><span>R4 revalidation</span><strong>{opportunity.r4_next_validation_at ? formatDateTime(opportunity.r4_next_validation_at) : "Required"}</strong><small>Commercial authority expires independently of the invalidation worker.</small></div>
