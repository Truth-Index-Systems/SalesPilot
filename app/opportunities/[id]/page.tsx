...
function statusLabel(status: string) {
  return ({ BUILDING: "Still assembling", READY: "Ready for review", NEEDS_CONTACT: "Needs contact", NEEDS_EVIDENCE: "Needs evidence", LOW_PRIORITY: "Low priority", APPROVED: "Approved", REJECTED: "Not selected", ENGAGED: "Engaged" } as Record<string, string>)[status] ?? status;
}

function componentLabel(key: string) {
  return ({ companyFit: "Company fit", operationalFit: "Operational fit", buyingAuthority: "Buying authority", contactability: "Contactability", evidenceQuality: "Evidence quality", commercialValue: "Commercial value", urgency: "Urgency" } as Record<string, string>)[key] ?? key;
}
...
export default async function OpportunityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const route = opportunity.primary_contact_email || opportunity.primary_route_email;
  ...
  return <AppShell title={opportunity.company_name} user={user} workspaceStats={{ campaigns: campaigns.length, companies: new Set(all.map(row => row.company_id)).size, replies: 0, opportunities: all.length }}>
    ...
    <Card className="opportunity-detail-hero">
      <div>
        ...
        <div className="opportunity-hero-actions"><a className="button secondary" href={opportunity.company_website_url} target="_blank" rel="noreferrer">Official company website <ExternalLink size={15}/></a>{opportunity.primary_contact_linkedin_url && <a className="button secondary" href={opportunity.primary_contact_linkedin_url} target="_blank" rel="noreferrer">Contact LinkedIn <ExternalLink size={15}/></a>}</div>
      </div>
    ...
    <div className="grid cols-2 section">
      ...
      <Card>
        <div className="card-title">Workspace decision</div><div className="card-subtitle">Approve the complete opportunity, not isolated company or contact records.</div>
    ...
    <Card className="section">
      <div className="section-head"><div><div className="card-title">Opportunity evidence</div><div className="card-subtitle">Company and contact evidence are brought together while remaining linked to their original repositories.</div></div><span className="badge green">{Number(opportunity.company_evidence_count) + Number(opportunity.contact_evidence_count)} sources</span></div>
      <div className="opportunity-evidence-columns section"><div><h3>Company evidence</h3><div className="evidence-list">{opportunity.company_evidence.map((evidence: any) => <div className="evidence-item" key={evidence.id}><div><strong>{evidence.claim}</strong>{evidence.excerpt && <p>“{evidence.excerpt}”</p>}<span>{evidence.sourceTitle || evidence.sourceDomain || "Official company source"}</span></div><a href={evidence.sourceUrl} target="_blank" rel="noreferrer"><ExternalLink size={16}/></a></div>)}</div></div><div><h3>Contact evidence</h3><div className="evidence-list">{opportunity.contact_evidence.length ? opportunity.contact_evidence.map((evidence: any) => <div className="evidence-item" key={evidence.id}><div><strong>{evidence.claim}</strong>{evidence.excerpt && <p>“{evidence.excerpt}”</p>}<span>{evidence.sourceTitle || evidence.sourceKind || "Official contact source"}</span></div><a href={evidence.sourceUrl} target="_blank" rel="noreferrer"><ExternalLink size={16}/></a></div>) : <div className="verified-empty"><span>No contact evidence has been persisted yet.</span></div>}</div></div></div>
    </Card>
