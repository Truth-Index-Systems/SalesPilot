...
function safePublicError(code: string, status: string, storedMessage: string | null) {
  ...
  if (code === "AI_GOVERNANCE_BLOCKED") {
    if (governanceReason === "PLATFORM_DISABLED") return { code:"AI_PLATFORM_PAUSED", title:"AI research is currently paused", message:"MarketRoute's deployment-level AI safety gate is disabled, so no OpenAI request was made.", hint:"Enable the platform gate in Vercel, then manage workspace access from Settings → AI governance." };
    if (governanceReason === "AUTONOMY_DISABLED") return { code:"AI_WORKSPACE_PAUSED", title:"AI research is paused for this workspace", message:"The workspace AI switch is off, so MarketRoute stopped before using any credit.", hint:"An owner or administrator can enable it in Settings → AI governance." };
    return { code:"AI_BUDGET_BLOCKED", title:"AI research stopped at its safety limit", message:"MarketRoute blocked this request before OpenAI because a daily request or cost limit was reached.", hint:"Review today's usage and limits in Settings → AI governance." };
  }
  if (code === "INVALID_AI_OUTPUT") return { code:"ANALYSIS_INTERRUPTED", title:status === "FAILED_TERMINAL" ? "MarketRoute could not complete this analysis" : "Analysis paused safely", message:"The AI response did not complete in the required structured format. No incomplete result was saved.", hint:status === "FAILED_RETRYABLE" ? "MarketRoute saved the job and can retry this stage safely." : "Start the analysis again." };
  if (code.startsWith("WEBSITE_")) return { code, title:"Website analysis could not complete", message:"MarketRoute could not read enough public website information to complete the analysis.", hint:status === "FAILED_RETRYABLE" ? "The saved job can retry without losing completed work." : "Check that the website is publicly reachable, then try again." };
  return { code:"ANALYSIS_INTERRUPTED", title:status === "FAILED_TERMINAL" ? "Analysis could not complete" : "Analysis paused safely", message:"MarketRoute encountered a technical interruption. No partial AI result was exposed.", hint:status === "FAILED_RETRYABLE" ? "The saved job can retry this stage safely." : "Try the analysis again." };
}
