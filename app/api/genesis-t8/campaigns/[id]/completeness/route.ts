...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ ok: true, completeness: sellerContext.completeness });
  } catch (error) {
    const message = error instanceof Error ? error.message : "GENESIS_COMPLETENESS_UNAVAILABLE";
