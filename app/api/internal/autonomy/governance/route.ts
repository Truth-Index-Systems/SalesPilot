...
const Schema=z.object({autonomyEnabled:z.boolean(),dailyRequestLimit:z.number().int().min(0).max(100000),dailyCostLimitUsd:z.number().min(0).max(100000),campaignDailyRequestLimit:z.number().int().min(0).max(100000),initialContactBurstSize:z.number().int().min(1).max(5)});
