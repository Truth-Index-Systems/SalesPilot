...
function safeError(error: unknown) {
  ...
  if (error instanceof Error && error.message === "CAMPAIGN_DATABASE_NOT_CONFIGURED") return { status: 503, body: { code: "CAMPAIGN_STORAGE_NOT_READY", title: "Campaign saving is not configured", message: "MarketRoute cannot save campaigns until the protected workspace connection is completed.", hint: "Complete the Supabase environment setup, then try again." } };
  return { status: 500, body: { code: "CAMPAIGN_LAUNCH_FAILED", title: "Campaign could not be launched", message: "MarketRoute could not save this campaign.", hint: "Please try again. Your selected strategy is still available." } };
}
