...
export default async function FounderDashboardPage({searchParams}:{searchParams:Promise<{range?:string}>}){
  ...
  return <main className="founder-dashboard-shell cie-dashboard-shell">
    ...
    <div className="founder-dashboard-content cie-dashboard-content">
      ...
      <section className="founder-grid founder-grid-equal cie-main-grid">
        <article className="founder-panel"><div className="founder-panel-head"><div><span>Reachability</span><h2>Route & contact authority</h2></div><b className="founder-live-badge">CIE-R5/R6</b></div><div className="cie-reachability-grid"><div><span>Bindings</span><strong>{number(cie.reachability.authoritativeDecisions)}</strong></div><div><span>READY</span><strong>{number(cie.reachability.ready)}</strong></div><div><span>Named contact</span><strong>{number(cie.reachability.namedContact)}</strong></div><div><span>Organisation route</span><strong>{number(cie.reachability.organisational)}</strong></div><div><span>Multi-contact frontier</span><strong>{number(cie.reachability.multiContactFrontier)}</strong></div><div><span>Awaiting binding</span><strong>{number(cie.reachability.awaitingBinding)}</strong></div></div><p className="founder-method-note">Categorical CIE authority only. No weighted route or contact score is shown.</p></article>
        <article className="founder-panel"><div className="founder-panel-head"><div><span>System health</span><h2>Constitutional operating status</h2></div><b className="founder-live-badge">Architecture frozen</b></div><div className="cie-health-grid">{systemHealth.map(([label,status])=><div key={label}><i className={`health-${status.toLowerCase()}`}/><span>{label}</span><strong>{status}</strong></div>)}</div></article>
      </section>

      <section className="founder-panel cie-company-table-panel"><div className="founder-panel-head"><div><span>Company intelligence</span><h2>Deepest researched companies</h2></div><strong>{cie.researchDensity.average.toFixed(1)}%</strong></div><div className="founder-table-wrap"><table className="founder-table cie-company-table"><thead><tr><th>Company</th><th>Density</th><th>Truth</th><th>Confidence</th><th>Truth state</th><th>Missing</th><th>Contradicted</th></tr></thead><tbody>{cie.companies.map(row=><tr key={row.entityId}><td><strong>{row.name}</strong></td><td><div className="cie-table-density"><i style={{width:`${row.coverage}%`}}/><span>{row.coverage.toFixed(1)}%</span></div></td><td>{row.truthIndex.toFixed(1)}</td><td>{row.confidence.toFixed(1)}%</td><td><span className={`cie-pill ${row.reviewState.toLowerCase()}`}>{row.reviewState.replaceAll("_"," ")}</span></td><td>{row.missing}</td><td>{row.contradicted}</td></tr>)}</tbody></table></div></section>
