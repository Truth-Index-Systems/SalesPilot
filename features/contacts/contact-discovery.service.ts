...
export async function runNextContactDiscovery():Promise<{processed:boolean;sessionId?:string;saved?:number}> {
  ...
  try{
    ...
    await databaseRequest("rpc/update_contact_discovery_progress",{method:"POST",body:JSON.stringify({p_session_id:job.session_id,p_stage:"RESEARCHING",p_progress:25})});
    ...
    await databaseRequest("rpc/update_contact_discovery_progress",{method:"POST",body:JSON.stringify({p_session_id:job.session_id,p_stage:"VALIDATING",p_progress:70,p_candidates:result.contacts.length})});
    if(result.contacts.length===0) throw new Error("CONTACT_DISCOVERY_NO_VERIFIED_CONTACTS");
    ...
    if(saved===0) throw new Error("CONTACT_DISCOVERY_NO_VERIFIED_CONTACTS");
