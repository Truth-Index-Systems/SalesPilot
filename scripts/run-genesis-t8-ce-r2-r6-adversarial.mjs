...
const opp=(state="ACTIONABLE",o={})=>{
 const viability=state==="NOT_VIABLE"?"ELIMINATED":state==="COMMERCIAL_REALITY_UNRESOLVED"?"UNRESOLVED":"SURVIVES";
 const routeState=o.routeState??(state==="STRANDED"?"BLOCKED":state==="VIABLE_BUT_UNRESOLVED"?"UNKNOWN":"DIRECT");
 const routeTargetMode=o.routeTargetMode??(state==="ACTIONABLE_WITHOUT_NAMED_CONTACT"?"ORGANISATION":"PERSON");
 const reasonCode=state==="NOT_VIABLE"?"COMMERCIAL_ELIMINATION":state==="COMMERCIAL_REALITY_UNRESOLVED"?"COMMERCIAL_UNRESOLVED":state==="STRANDED"?"ROUTE_BLOCKED":state==="VIABLE_BUT_UNRESOLVED"?"CONTACT_OR_ROUTE_UNKNOWN":state==="ACTIONABLE_WITHOUT_NAMED_CONTACT"?"ORGANISATIONAL_OR_INTERMEDIARY_ROUTE_AVAILABLE":"CONTACT_AND_ROUTE_AVAILABLE";
 const nonSurvivor=viability!=="SURVIVES"?{commercialCoherence:0,commercialStability:0}:{};
 return {opportunityId:"opp1",targetEntityId:"target1",realisation:{state,commercial:commercial({...o.commercial,...nonSurvivor,viability}),contactState:o.contactState??"APPROPRIATE",routeState,routeTargetMode,actionable:state==="ACTIONABLE"||state==="ACTIONABLE_WITHOUT_NAMED_CONTACT",reasonCode}};
};
...
// Portfolio behaviour
test("portfolio decision impact outranks supplied R5 order",()=>{const o1={...opp("ACTIONABLE"),opportunityId:"top",targetEntityId:"t-top"};const o2={...opp("VIABLE_BUT_UNRESOLVED",{routeState:"UNKNOWN"}),opportunityId:"lower",targetEntityId:"t-lower"};const p1=prop();const p2=prop();const s=m.selectNextPortfolioResearch([o1,o2],new Map([["top",p1],["lower",p2]]),new Map([["top",[q("top-q","CONSTRAINT",{constraintId:"c1"})]],["lower",[q("lower-route","ROUTE")]]]));assert.equal(s.next.opportunityId,"lower")});
test("lower-ranked viability unknown outranks top assurance gap",()=>{const top={...opp("ACTIONABLE"),opportunityId:"top-vs-assurance",targetEntityId:"t1"};const lower={...opp("COMMERCIAL_REALITY_UNRESOLVED",{commercial:{viability:"UNRESOLVED"}}),opportunityId:"lower-viability",targetEntityId:"t2"};const topProp=prop();const lowerProp=prop({viability:"UNRESOLVED",unresolvedBoundaryConstraintIds:["b1"]});const out=m.selectNextPortfolioResearch([top,lower],new Map([[top.opportunityId,topProp],[lower.opportunityId,lowerProp]]),new Map([[top.opportunityId,[q("assurance","CONSTRAINT",{constraintId:"c1"})]],[lower.opportunityId,[q("viability","CONSTRAINT",{constraintId:"b1"})]]]));assert.equal(out.next.opportunityId,"lower-viability");assert.equal(out.next.impactClass,"VIABILITY_PIVOTAL")});
...
console.log(`\nGenesis T8 CE-R2 R6 adversarial runtime: ${pass}/39 passed.`);
