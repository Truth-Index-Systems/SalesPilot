...
const executor=fs.readFileSync(new URL('lib/pipeline/executor.ts',root),'utf8');
const summary=fs.readFileSync(new URL('lib/pipeline/result-summary.ts',root),'utf8');
...
const checks=[
 ...
 ['worker outcome union',executor.includes('ROUTE_EXPANSION_QUEUED')&&executor.includes('ROUTE_RESEARCH_EXHAUSTED')],
 ['result summary union',summary.includes('ROUTE_RESEARCH_EXHAUSTED')],
