...
check("9D graph version is explicit", /GENESIS_T8_9D_GRAPH_VERSION\s*=\s*"1\.[01]\.0"/.test(graph));
...
check("unknown relationship is absence law", graph.includes("RELATIONSHIPS_ARE_FIRST_CLASS_EDGES") && graph.includes("EDGE_TOKEN_MISSING"));
...
check("opaque model embeddings are rejected by law", graph.includes("PROJECTIONS_USE_AI_CANONICALISED_OR_ONTOLOGY_COORDINATES_NOT_MODEL_EMBEDDINGS"));
check("AI cannot write truth dimension", graph.includes("AI_MAY_NOT_WRITE_TRUTH_DIMENSION_PROJECTIONS"));
check("TI alone owns truth dimension", graph.includes("TRUTH_DIMENSION_REQUIRES_AUTHORISED_TRUTH_AUTHORITY"));
check("truth ownership table is TI-only", /TRUTH:\s*\["TRUTH_AUTHORITY"\]/.test(graph));
check("truth projection requires qualified token", graph.includes("TRUTH_PROJECTION_REQUIRES_QUALIFIED_TOKEN"));
