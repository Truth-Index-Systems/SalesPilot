...
check("constraint mathematics version explicit",src.includes('GENESIS_T8_CONSTRAINT_MATHEMATICS_VERSION = "1.0.0"'));
check("R2 build identity explicit",src.includes('GENESIS_T8_CE_R2_R2_BUILD = "R2-BUILD1"'));
check("semantic polarity is categorical",src.includes('"SUPPORTS_REALITY"')&&src.includes('"OPPOSES_REALITY"')&&src.includes('"UNKNOWN"'));
check("TI truth input includes probability",src.includes("probability: number"));
check("TI truth input includes confidence",src.includes("confidence: number"));
...
check("truth signal equation implemented",src.includes("(2 * p - 1) * c"));
check("coverage excluded from truth force",src.includes("Coverage is intentionally absent from this equation"));
check("knowledge equation is coverage times confidence",src.includes("return clamp01(coverage) * clamp01(confidence)"));
