...
export const GENESIS_G82_EXPANSION_RESEARCH_VERSION = "G8.2-MRTI2-B8.3.3-BREADTH-DECOMPOSED-3.0" as const;
...
const CompanySchema = z.object({
  ...
  domain: z.string().min(3).max(240),
  sector: z.string().max(180).nullable(),
  geography: z.string().max(180).nullable(),
  offering: z.string().max(320).nullable(),
  customerMarket: z.string().max(320).nullable(),
...
async function recoverCompletedExpansionResponse(params:{
  ...
  const rows=await databaseRequest<CompletedExpansionCheckpoint[]>(
    `ai_background_responses?job_type=eq.GENESIS_G82_EXPANSION&job_id=eq.${encodeURIComponent(params.jobId)}&request_scope=like.genesis-g82-expansion-v3:*&status=eq.completed&response_json=not.is.null&select=response_id,ledger_id,response_json,request_scope,created_at&order=created_at.asc&limit=12`,
  ).catch(()=>[]);
  ...
  for(const row of rows){
    ...
    try{
      const gateway=await parseStructuredAiResponse({response:row.response_json,schema:ExpansionResultSchema,jsonSchema:expansionJsonSchema,schemaName:"genesis_g82_expansion_v1",apiKey:params.apiKey,model:params.model});
...
export async function researchGenesisG82IndustryExpansion(input:{
  ...
  const baseScope=`genesis-g82-expansion-v3:${fingerprint}`;
      ...
      response=await fetchResumableOpenAIResponse({apiKey,task:"GENESIS_G82_EXPANSION",organisationId,campaignId:null,jobType:"GENESIS_G82_EXPANSION",jobId:input.jobId,requestScope,model,ledgerId:reservation.ledgerId},{
        ...
        body:JSON.stringify({
          ...
          instructions:[
            ...
            "Write concise British English and return exact JSON only. Prompt policy: genesis-g82-expansion/v3-breadth-decomposed.",
          ].join(" "),
