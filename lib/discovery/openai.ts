...
const schema = {
  ...
  properties: {
    ...
    companies: { type: "array", minItems: 1, maxItems: 20, items: { type: "object", additionalProperties: false,
      required: ["name", "websiteUrl", "country", "industry", "summary", "confidence", "matchLabel", "fitBreakdown", "why", "uncertainties", "riskFlags", "evidence"],
      properties: {
        name: { type: "string" }, websiteUrl: { type: "string" }, country: { type: "string" }, industry: { type: "string" }, summary: { type: "string" }, confidence: score,
        matchLabel: { type: "string", enum: ["Strongest match", "Strong match", "Good match"] },
        fitBreakdown: { type: "object", additionalProperties: false, required: ["industryFit", "audienceFit", "operationalFit", "geographyFit", "commercialFit"], properties: { industryFit: score, audienceFit: score, operationalFit: score, geographyFit: score, commercialFit: score } },
        why: { type: "array", items: { type: "string" } }, uncertainties: { type: "array", items: { type: "string" } }, riskFlags: { type: "array", items: { type: "string" } },
...
export async function discoverCompanies(input: { campaign: Record<string, unknown>; business: Record<string, unknown>; customerWebsite?: string | null }) {
  ...
  const response = await fetch(ENDPOINT, { method: "POST", cache: "no-store", headers: { authorization: `Bearer ${apiKey}`, "content-type": "application/json" }, body: JSON.stringify({
    model,
    instructions: `You are SalesPilot Company Discovery. Use web search to find real operating B2B companies that genuinely match the approved outbound sales campaign. Return only official company websites and evidence from those official domains. Do not invent employee counts, technology usage, operational problems, buyer intent or private information. Exclude the customer's own company, directories, agencies listing clients, news articles and duplicate domains. Score industry fit, audience fit, operational fit, geography fit and commercial fit independently. Record genuine uncertainties and risk flags instead of hiding them. Prefer 8-12 high-confidence matches. British English.`,
    input: JSON.stringify({ approvedCampaign: input.campaign, approvedBusinessUnderstanding: input.business }),
    tools: [{ type: "web_search_preview", search_context_size: "medium" }],
    text: { format: { type: "json_schema", name: "salespilot_company_discovery_v2", strict: true, schema } },
    max_output_tokens: 11000, store: false,
  }) });
