  ...
  for (let terminalGeneration = 0; terminalGeneration < 4; terminalGeneration += 1) {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "MR-R1 BUILD 7 AUTHORITY: AI supplies semantic research/explanation only; it must not calculate contact, route or opportunity ranking. Genesis/Truth Index/UDOSIB own authoritative decisions. ADVISES BUT DOES NOT DECIDE: You may assess account fit, explain evidence and recommend which discovered companies deserve attention. You do NOT approve/reject companies in workflow state, choose contacts, choose routes/channels, set scheduler priority, decide Opportunity readiness, or create outreach. MarketRoute validates/persists; later executives own account access and engagement.",
        ...
        "GENESIS CONSTRAINT CONTRACT: business.genesisConstraintContracts is immutable seller-context state. Consume its BOUNDARY, SUPPORTING, LIMITING and UNKNOWN classes exactly as supplied. Never reclassify or invent a seller constraint. A BOUNDARY may eliminate a candidate only when the evaluated reality actually violates that applicable boundary; LIMITING constraints restrict preference but are not automatic rejection; SUPPORTING constraints add coherence but cannot override a violated boundary; UNKNOWN constraints identify research debt and must not be treated as false.",
