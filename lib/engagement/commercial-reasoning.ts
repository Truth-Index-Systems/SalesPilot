...
export type CommercialReasoningWorkerResult={processed:boolean;outcome:"NO_JOB"|"COMPLETED"|"FAILED_RETRYABLE"|"SUPERSEDED";analysisId?:string;engagementId?:string};
...
export async function runNextCommercialReasoning(schedulerRunId:string):Promise<CommercialReasoningWorkerResult>{
  const claimed=await databaseRequest<Claim[]>("rpc/claim_engagement_commercial_reasoning_owned",{method:"POST",body:JSON.stringify({p_scheduler_run_id:schedulerRunId})});
  ...
  try{
    ...
    await databaseRequest("rpc/complete_engagement_commercial_reasoning_owned",{method:"POST",body:JSON.stringify({p_analysis_id:job.analysis_id,p_scheduler_run_id:schedulerRunId,p_output_json:generated.result,p_prompt_version:generated.result.promptVersion,p_schema_version:generated.result.schemaVersion,p_confidence:generated.result.confidence,p_model:generated.model})});
    ...
    if(isPipelineOwnershipLost(error)) return {processed:false,outcome:"SUPERSEDED",analysisId:job.analysis_id,engagementId:job.engagement_id};
    ...
    await databaseRequest("rpc/fail_engagement_commercial_reasoning_owned",{method:"POST",body:JSON.stringify({p_analysis_id:job.analysis_id,p_scheduler_run_id:schedulerRunId,p_error:error instanceof Error?error.message:"COMMERCIAL_REASONING_FAILED"})}).catch(()=>undefined);
