...
export async function reasonAboutEngagement(input:{organisationId:string;campaignId:string;schedulerRunId:string;analysisId:string;context:Record<string,unknown>}):Promise<{result:CommercialReasoning;model:string}>{
  ...
  const model=resolveOpenAIModel("analysis").model; const compactContext=compactForAi(input.context,{evidenceLimit:6,depth:6}) as Record<string,unknown>; const fingerprint=stableFingerprint({prompt:"commercial-reasoning/v1-cost-optimised",model,compactContext}); const startedAt=Date.now();
  ...
  ].join(" "),input:JSON.stringify(compactContext),text:{format:{type:"json_schema",name:"salespilot_commercial_reasoning_v1",strict:true,schema:commercialReasoningJsonSchema}},max_output_tokens:2600,store:false})});}
