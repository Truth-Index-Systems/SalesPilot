  ...
  try {
    response = await fetchResumableOpenAIResponse({ apiKey, task: "G5_COMMERCIAL_REASONING", organisationId: input.organisationId, campaignId: input.campaignId, jobType: "COMMERCIAL_REASONING", jobId: input.strategyId, requestScope: `g5-commercial-reasoning:${requestFingerprint}`, model, ledgerId: reservation.ledgerId }, {
      ...
      body: JSON.stringify({
        model,
        instructions: [
          "ROLE: Chief Revenue Officer / Executive Deal Strategist for MarketRoute.",
          ...
          "ACCOUNTABLE FOR: Convert the current evidence-qualified R4 commercial reality and persisted CIE-R5/R6 access authority into the account-specific commercial thesis: company relevance, why the authorised access context is commercially relevant, timing, likely problem, plausible consequence, credible outcome, buyer concern, objection and the smallest sensible next commitment.",
          "MR-R1 BUILD 7 AUTHORITY: AI supplies semantic research/explanation only; it must not calculate contact, route or opportunity ranking. Genesis/Truth Index/UDOSIB own authoritative decisions. ADVISES BUT DOES NOT DECIDE: You recommend the commercial thesis and next-conversation objective. You do NOT create/rank new routes, choose the execution channel, write wording, set confidence thresholds, approve outreach, change workflow state or send anything. MarketRoute owns deterministic gates; Channel Strategy and Executive Communications own those later decisions.",
          ...
          "R4 commercial reality and the persisted CIE-R5/R6 route/contact authority are immutable inputs for this task. Consume the approved opportunity, Business DNA, campaign strategy and supplied evidence. Never rediscover, overwrite, score, rank or contradict the authorised route.",
          "Treat the supplied CIE-R5 route as the sole route authority. Never invent or substitute a person, route, channel, buying path, relationship, budget, trigger or company fact.",
          ...
          "Do not write an email, LinkedIn message, phone script, subject line or switchboard script. Do not turn whyThisRoute into a new route-selection decision; explain only the commercial relevance of the persisted CIE-R5 route supplied in context.",
