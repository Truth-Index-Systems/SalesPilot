...
export async function syncEngagementStrategies(schedulerRunId: string): Promise<EngagementStrategySyncResult> {
  const rows = await databaseRequest<EngagementStrategySyncResult[]>("rpc/sync_engagement_strategies_owned", {
    method: "POST",
...
export async function recordEngagementStage(input: { engagementId: string; schedulerRunId: string; stage: string; state: string; reason?: string; worker?: string; metadata?: Record<string, unknown> }) {
  await databaseRequest("rpc/record_engagement_pipeline_stage_owned", { method: "POST", body: JSON.stringify({
...
export async function reconcileEngagementFailures(schedulerRunId: string): Promise<number> {
  return Number(await databaseRequest<number>("rpc/reconcile_engagement_pipeline_failures_owned", { method: "POST", body: JSON.stringify({ p_scheduler_run_id: schedulerRunId }) }));
}
...
export async function syncEngagementLearningGuidance(schedulerRunId: string): Promise<EngagementLearningGuidanceResult> {
  const rows = await databaseRequest<EngagementLearningGuidanceResult[]>("rpc/sync_engagement_learning_guidance_owned", {
