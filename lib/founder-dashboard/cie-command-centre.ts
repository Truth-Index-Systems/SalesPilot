...
export async function getCieFounderCommandCentre():Promise<CieFounderCommandCentre>{
  ...
  const [entities,snapshots,r4,r6,r7,repairs,depthJobs,expansionJobs,recentEntities,evidenceToday,truthToday]=await Promise.all([
    ...
    databaseRequest<TruthTodayRow[]>(`genesis_g8_truth_v2_snapshots?select=entity_id,calculated_at&calculated_at=gte.${encodeURIComponent(todayIso)}&limit=10000`).catch(()=>[]),
  ]);
