...
export async function researchRouteIntelligence(input:{organisationId:string;campaignId:string;schedulerRunId?:string|null;jobId:string;company:Record<string,unknown>;campaign:Record<string,unknown>;business:Record<string,unknown>;routeExpansionPass?:number}){
  ...
  const fingerprint=stableFingerprint({prompt:"contact-discovery/v4-executive-account-mapping",model,compactInput});
  ...
  try{response=await fetch(ENDPOINT,{method:"POST",cache:"no-store",signal:AbortSignal.timeout(Number(input.routeExpansionPass??0)===0?240_000:180_000),headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({
    ...
    instructions:[
      ...
      "EXECUTIVE ACCOUNTABILITY: Map only enough of the organisation to understand functional ownership, budget authority, economic influence, procurement/routing power and executable access paths. The output should be useful to a senior seller walking into an account-planning meeting tomorrow morning.",
      ...
      "ROUTING-POWER PRINCIPLE: Score not just authority but relevance, accessibility and ability to route the seller internally. Ask: if this person is not the final buyer, are they credibly positioned to get us to the right one?",
      ...
      "Search official public sources for useful company-domain business email routes as well as named routes. Rank them by likely reader, operational relevance, response likelihood and routing power.",
      ...
      "Generate route diversity where evidence supports it: operational, commercial, transformation, procurement, technical, executive, regional, introduction and fallback. Score the ROUTE across authority, accessibility, relevance, evidence, resilience and confidence-not merely the person's seniority.",
      ...
      "Return at most 8 well-supported people, 10 commercial routes and 10 company channels. Write calm British English.",
      ...
      "Prompt policy: contact-discovery/v4-executive-account-mapping."
    ].join(" "),
