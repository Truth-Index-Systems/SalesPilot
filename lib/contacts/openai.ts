...
export async function researchRouteIntelligence(input:{organisationId:string;campaignId:string;schedulerRunId?:string|null;jobId:string;company:Record<string,unknown>;campaign:Record<string,unknown>;business:Record<string,unknown>;routeExpansionPass?:number}){
  ...
  try{response=await fetch(ENDPOINT,{method:"POST",cache:"no-store",signal:AbortSignal.timeout(150_000),headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({
    ...
    instructions:[
      ...
      "On the FIRST pass be extensive: map relevant departments/business units, infer supported buying centres and likely hierarchy, generate several independent buying paths, then investigate the strongest people and channels for each path. A single contact is not a route strategy.",
    ...
    text:{format:{type:"json_schema",name:"salespilot_contact_discovery_v3",strict:true,schema}},max_output_tokens:12000,store:false
