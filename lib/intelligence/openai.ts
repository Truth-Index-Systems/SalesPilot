...
export async function analyseBusinessGrowth(params: CommonParams & {core:CoreBusinessDnaEnvelope}):Promise<GrowthStrategyEnvelope>{
  ...
  return runPhase({...params,phase:"growth",instructions,input:`PERSISTED CORE BUSINESS DNA:\n${coreInput}`,jsonSchema:growthStrategyJsonSchema,schemaName:"marketroute_growth_strategy",maxOutputTokens:Math.min(profile.maxOutputTokens,Math.max(4_200,Math.min(20_000,Number(process.env.MARKETROUTE_BUSINESS_ANALYSIS_GROWTH_MAX_OUTPUT_TOKENS??String(profile.maxOutputTokens))))),reasoningEffort:"medium",estimatedCostUsd:Number(process.env.MARKETROUTE_BUSINESS_ANALYSIS_GROWTH_ESTIMATED_COST_USD??"0.06"),fingerprintData:{prompt:"business-discovery-growth/v1",cacheKey:`${aiPromptCacheKey("BUSINESS_ANALYSIS")}:growth`,core:params.core.payload},canonicalise:canonicalGrowth});
}
