...
-- Gives depth enrichment its own governance/background namespace while preserving
-- the shared heavy-work concurrency limits and workspace budget authority.
...
alter table public.ai_usage_ledger add constraint ai_usage_ledger_job_type_check
  check (job_type in ('BUSINESS_ANALYSIS','COMPANY_DISCOVERY','CONTACT_DISCOVERY','OUTREACH','COMMERCIAL_REASONING','REPLY_INTELLIGENCE','GENESIS_G8_REPAIR','GENESIS_G82_EXPANSION','GENESIS_G82_DEPTH'));
...
with policy as (
  ...
  select count(*) filter(where l.status='SUCCEEDED')::integer calls,
    ...
    count(*) filter(where l.status='SUCCEEDED' and ((q.organisation_id is null and q.campaign_id is null and q.company_id is null and q.id is not null) or x.id is not null or d.id is not null))::integer background_calls,
    coalesce(sum(case when ((q.organisation_id is null and q.campaign_id is null and q.company_id is null and q.id is not null) or x.id is not null or d.id is not null) then case when l.status='SUCCEEDED' then l.actual_cost_usd else l.estimated_cost_usd end else 0 end),0)::numeric background_cost
  from public.ai_usage_ledger l left join public.genesis_g8_discovery_repair_queue q on q.id=l.job_id left join public.genesis_g82_expansion_jobs x on x.id=l.job_id left join public.genesis_g82_depth_jobs d on d.id=l.job_id
  where l.organisation_id=p_system_organisation_id and l.job_type in ('GENESIS_G8_REPAIR','GENESIS_G82_EXPANSION','GENESIS_G82_DEPTH') and l.created_at>=date_trunc('day',now()) and l.status in ('RESERVED','SUCCEEDED','FAILED')
), customer_repair as (
...
comment on function public.reserve_ai_request(uuid,uuid,uuid,text,uuid,text,text,numeric) is
'G8.2 Depth identity hardening: GENESIS_G82_DEPTH has an isolated AI/background namespace under the same workspace and parallel governance.';

notify pgrst, 'reload schema';

-- Completion reconciliation: the pre-hardening depth worker could settle a job
-- COMPLETED with 0/0 even though its provider checkpoint later completed. Requeue
-- only those exact zero-output jobs that have durable completed depth research and
-- still have no contact/route membership. Existing enriched jobs are untouched.
update public.genesis_g82_depth_jobs j
set status='QUEUED',
    lease_token=null,
    lease_expires_at=null,
    worker_id=null,
    completed_at=null,
    last_error='GENESIS_G82_DEPTH_COMPLETED_RESPONSE_RECONCILIATION',
    updated_at=now()
where j.status='COMPLETED'
  and j.contacts_persisted=0
  and j.routes_persisted=0
  and not exists (
    select 1 from public.genesis_g82_expansion_membership m
    where m.target_id=j.target_id and m.entity_type in ('contact','route')
  )
  and exists (
    select 1 from public.ai_background_responses b
    where b.job_id=j.id
      and b.request_scope like 'genesis-g82-depth:%'
      and b.status='completed'
      and b.response_json is not null
  );
