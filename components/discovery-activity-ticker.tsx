...
export function DiscoveryActivityTicker({campaignId,initialDiscovery,initialActivities,initialCompanyCount}:{campaignId:string;initialDiscovery:Discovery|null;initialActivities:Activity[];initialCompanyCount:number}){
 ...
 return <div className="discovery-live-feed" aria-live="polite">
  <div className="discovery-live-head"><span className={running?"live-dot active":"live-dot"}/><strong>{running?"SalesPilot is working":"Latest discovery activity"}</strong><small>{companyCount} compan{companyCount===1?"y":"ies"} saved</small></div>
  <div className="discovery-live-items">{visible.map((item,index)=><div className="discovery-live-item" key={item.id}><CheckCircle2 size={16}/><div><strong>{item.title}</strong>{item.description&&<span>{item.description}</span>}</div><time>{relative(item.occurred_at)}</time></div>)}</div>
 </div>;
}
function relative(value:string){const seconds=Math.max(0,Math.floor((Date.now()-new Date(value).getTime())/1000));if(seconds<60)return "Just now";const minutes=Math.floor(seconds/60);if(minutes<60)return `${minutes}m ago`;return `${Math.floor(minutes/60)}h ago`;}
