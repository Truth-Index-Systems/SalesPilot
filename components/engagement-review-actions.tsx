...
export function EngagementReviewActions({id,initial}:{id:string;initial:{subject:string;opening:string;personalisation:string;valueProposition:string;callToAction:string}}){
  ...
  return <div className="engagement-review-panel">
    ...
    <div className="company-review-actions">
      {!editing?<><button className="button primary" disabled={!!busy} onClick={()=>act("APPROVED")}>{busy==="APPROVED"?"Approving…":"Approve outreach"}</button><button className="button secondary" disabled={!!busy} onClick={()=>setEditing(true)}>Edit</button></>:<><button className="button primary" disabled={!!busy} onClick={()=>act("EDITED")}>{busy==="EDITED"?"Saving…":"Save edits and approve"}</button><button className="button secondary" disabled={!!busy} onClick={()=>setEditing(false)}>Cancel edit</button></>}
