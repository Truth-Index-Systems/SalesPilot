...
const relative=(iso:string,referenceMs:number)=>{const ms=referenceMs-new Date(iso).getTime();const min=Math.max(0,Math.round(ms/60000));if(min<1)return"just now";if(min<60)return`${min}m ago`;const h=Math.round(min/60);if(h<24)return`${h}h ago`;return`${Math.round(h/24)}d ago`;};
const activityTime=(iso:string)=>new Intl.DateTimeFormat("en-GB",{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:"Europe/London"}).format(new Date(iso));
const qualityLabel=(v:number)=>v>=.72?"High":v>=.45?"Medium":"Low";

export function GenesisG8ReviewWorkspace({tasks,summary,activity,renderedAt}:{tasks:ReviewTask[];summary:Summary;activity:Activity[];renderedAt:string}){
  const referenceMs=useMemo(()=>new Date(renderedAt).getTime(),[renderedAt]);
  ...
  return <section className="founder-panel founder-g8-review-panel founder-review-workspace">
    <div className="founder-review-layout">
        ...
        <div className="founder-g8-review-list enhanced">
          {sorted.length?sorted.map(task=>{
            ...
            return <article key={task.id} className="founder-g8-review-card enhanced">
              ...
              <div className="founder-review-metric-row"><div><span>Confidence</span><strong>{Number(task.confidence).toFixed(1)}%</strong><i><b style={{width:`${Math.max(2,Number(task.confidence))}%`}}/></i></div><div><span>Coverage</span><strong>{Number(task.coverage).toFixed(1)}%</strong><i><b style={{width:`${Math.max(2,Number(task.coverage))}%`}}/></i></div><div><span>Evidence items</span><strong>{evidence.length}</strong><small>verifiable sources</small></div><div><span>First seen</span><strong>{relative(task.created_at,referenceMs)}</strong></div><div><span>Last evidence</span><strong>{relative(lastSeen,referenceMs)}</strong></div></div>
      ...
      <aside className="founder-review-sidebar">
        <div className="founder-activity-panel"><div className="founder-activity-head"><strong>Live activity feed</strong><span>● Live</span></div><div className="founder-activity-list">{activity.length?activity.slice(0,12).map(item=><div key={`${item.kind}-${item.refId}`}><time>{activityTime(item.occurredAt)}</time><div><span className={`activity-${item.kind.toLowerCase()}`}>{item.kind}</span><strong>{item.title}</strong><small>{item.detail}</small></div></div>):<div className="founder-empty compact">Genesis activity will appear here as the engine works.</div>}</div></div>
