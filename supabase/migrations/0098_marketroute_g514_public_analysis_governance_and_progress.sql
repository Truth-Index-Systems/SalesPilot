-- MarketRoute G5.1.4 — governed anonymous analysis + truthful monotonic progress.
-- Anonymous Business Analysis is an intentional public product surface, so it
-- receives its own bounded AI reservation lane instead of requiring a workspace.
-- Authenticated/workspace AI governance is unchanged.
...
begin
  ...
  -- One global transaction lock keeps anonymous reservations atomic across all
  -- serverless invocations. Per-browser entitlement is independently enforced
  -- before the Business Analysis job is created.
...
-- Keep progress monotonic when the same durable job is reclaimed. A background
-- response resume must never visually fall from 52% back to 8%.
...
begin
  update public.business_analysis_jobs baj set
    status='RUNNING',stage='READING_WEBSITE',progress=greatest(coalesce(baj.progress,0),8),attempt_count=baj.attempt_count+1,
...
-- Retryable failures are transport/governance implementation details. Preserve
-- the last truthful stage/progress rather than resetting the UI to zero.
