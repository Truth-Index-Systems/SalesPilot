-- Genesis G4.7.9: Company Discovery legacy recovery leak hardening.
--
-- Removes the final duplicate lease-recovery path from prepare_pipeline_work().
-- recover_pipeline_jobs() is now the single owner of expired worker recovery.
-- Also repairs impossible split-state rows produced by the legacy duplicate
-- recovery and keeps later replenishment failures non-blocking in the UI.

-- Repair impossible Company Discovery rows that the legacy prepare function
-- could manufacture by changing status without changing canonical job_state.
update public.discovery_sessions
set job_state=case when coalesce(attempt_count,0)>=5 then 'FAILED_TERMINAL' else 'FAILED_RETRYABLE' end,
    stage=case when coalesce(attempt_count,0)>=5 then 'NEEDS_ATTENTION' else 'TECHNICAL_RETRY' end,
    last_error_code=coalesce(last_error_code,'WORKER_LEASE_EXPIRED'),
    last_error_message=coalesce(last_error_message,'The worker lease expired before completion.'),
    next_retry_at=case when coalesce(attempt_count,0)>=5 then null else coalesce(next_retry_at,next_attempt_at,now()) end,
    next_attempt_at=case when coalesce(attempt_count,0)>=5 then null else coalesce(next_attempt_at,next_retry_at,now()) end,
    claimed_at=null,
...
where status='FAILED'
  ...
  and coalesce(last_error,'')='WORKER_LEASE_EXPIRED';

-- Same repair for the historical Contact Discovery table now used as the Route
-- Intelligence compatibility boundary.
update public.contact_discovery_sessions
set job_state=case when coalesce(attempt_count,0)>=8 then 'FAILED_TERMINAL' else 'FAILED_RETRYABLE' end,
    stage='PREPARING',
    result_status='FAILED',
    last_error_code=coalesce(last_error_code,'WORKER_LEASE_EXPIRED'),
    last_error_message=coalesce(last_error_message,'The worker lease expired before completion.'),
    next_retry_at=case when coalesce(attempt_count,0)>=8 then null else coalesce(next_retry_at,next_attempt_at,now()) end,
    next_attempt_at=case when coalesce(attempt_count,0)>=8 then null else coalesce(next_attempt_at,next_retry_at,now()) end,
    claimed_at=null,
    scheduler_run_id=null,
    lease_expires_at=null,
    updated_at=now()
...
begin
  ...
  -- Recovery is intentionally NOT performed here. recover_pipeline_jobs() is
  -- the single authority for lease expiry, retry classification and canonical
  -- job_state transitions. Duplicating recovery here previously created split
  -- states such as status='FAILED' + job_state='RUNNING'.
  v_company_recovered:=0;
  v_contact_recovered:=0;
  ...
  loop
    ...
    if v_session.id is null then
      ...
      if v_pending_companies=0
        ...
        update public.discovery_sessions
        set status='QUEUED',job_state='QUEUED',stage='PREPARING',progress=0,
            candidates_found=0,recommendations_saved=0,
            attempt_count=0,last_error=null,last_error_code=null,last_error_message=null,
            started_at=null,completed_at=null,
            next_attempt_at=now(),next_retry_at=null,
            claimed_at=null,scheduler_run_id=null,lease_expires_at=null,
            heartbeat_at=null,last_heartbeat_at=null,
            result_summary_json=coalesce(result_summary_json,'{}'::jsonb)
              || jsonb_build_object('technicalFailure',false,'expansionPending',false),
...
-- Replace the original G2 manual retry RPC. A manual retry must reset BOTH the
-- legacy status and the canonical job_state, clear stale ownership, and become
-- immediately claimable by the current scheduler.
...
begin
  ...
  where campaign_id=p_campaign_id
    ...
    and (status='FAILED' or job_state='FAILED_TERMINAL');
  return found;
end $$;
