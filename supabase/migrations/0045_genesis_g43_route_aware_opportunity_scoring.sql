-- Genesis G4.3: route-aware opportunity scoring.
-- Evolves opportunity ranking from contact authority toward commercial access quality.
-- Uses only persisted route/contact/evidence data; no AI or web requests are introduced.
-- Human decisions remain authoritative and low-scoring opportunities remain visible.
...
alter table public.opportunities
  ...
  add column if not exists route_quality integer check (route_quality between 0 and 100),
  add column if not exists route_confidence integer check (route_confidence between 0 and 100),
  add column if not exists recommended_entry_strategy text;
...
create index if not exists opportunities_route_quality_idx
  on public.opportunities(organisation_id,campaign_id,route_quality desc nulls last,opportunity_score desc nulls last);
...
select
  ...
  ch.confidence as primary_route_confidence,
  ch.response_likelihood as primary_route_response_likelihood,
  ch.campaign_relevance as primary_route_campaign_relevance,
  ch.channel_type as primary_route_channel_type,
  ...
  (select count(*) from public.company_contact_channels alt
    where alt.organisation_id=o.organisation_id and alt.campaign_id=o.campaign_id and alt.company_id=o.company_id
      and alt.deliverability_status not in ('UNDELIVERABLE','BOUNCED')) as available_route_count
...
left join lateral (
  select cch.id,cch.email_address,cch.verification_status,cch.routing_score,cch.confidence,
    cch.response_likelihood,cch.campaign_relevance,cch.channel_type,cch.likely_reader,
    cch.reason_selected,cch.source_url,cch.last_successful_reply_at
...
declare
  ...
  v_route_confidence integer;
  v_route_evidence integer;
  v_relationship_path integer;
  v_entry_strategy text;
...
begin
  ...
  loop
    ...
    -- Route confidence measures how strongly the route itself is supported.
    -- Route quality measures whether that supported route is commercially usable.
    v_route_evidence:=least(100,greatest(0,
      least(100,v_contact_evidence*25)+
      case when v_route.id is not null and nullif(v_route.source_url,'') is not null then 20 else 0 end
    ));
    v_relationship_path:=case
      when v_route.id is not null and v_route.verification_status='INTERNAL_CONFIRMED' then 100
      when v_route.id is not null and v_route.last_successful_reply_at is not null then 95
      when v_route.id is not null and v_route.channel_type='NAMED' then 70
      when v_contact.id is not null and nullif(v_contact.linkedin_profile_url,'') is not null then 55
      when v_route.id is not null then 45
      else 0 end;

    v_route_confidence:=least(100,greatest(0,round(
      coalesce(v_route.confidence,0)::numeric*0.40+
      coalesce(v_contact.overall_confidence,0)::numeric*0.25+
      v_route_evidence::numeric*0.20+
      case
        when v_route.verification_status='INTERNAL_CONFIRMED' then 100
        when v_route.verification_status='PUBLIC_VERIFIED' then 90
        when v_contact.email_status='VERIFIED' then 88
        when v_route.verification_status='PATTERN_LIKELY' or v_contact.email_status='LIKELY' then 65
        when nullif(v_contact.linkedin_profile_url,'') is not null then 50
        else 0 end::numeric*0.15
    )::integer));

    v_route_quality:=least(100,greatest(0,round(
      v_contactability::numeric*0.25+
      coalesce(v_route.response_likelihood,0)::numeric*0.20+
      v_buying_authority::numeric*0.20+
      v_route_confidence::numeric*0.15+
      coalesce(v_route.campaign_relevance,0)::numeric*0.10+
      v_relationship_path::numeric*0.10
    )::integer));

    v_entry_strategy:=case
      when v_route.id is not null and v_route.verification_status='INTERNAL_CONFIRMED'
        then 'Use the confirmed internal route and reference the established relationship.'
      when v_contact.id is not null and v_contact.email_status='VERIFIED' and nullif(v_contact.email_address,'') is not null
        then 'Approach the recommended decision-maker directly through the verified email route.'
      when v_route.id is not null and v_route.channel_type='NAMED'
        then 'Use the named email route as the primary entry point and support it with the available role evidence.'
      when v_route.id is not null and v_route.channel_type='DEPARTMENTAL'
        then 'Enter through the relevant department and ask to be routed to the recommended decision-maker.'
    ...
    v_score:=least(100,greatest(0,round(
      v_company_fit::numeric*0.24+
      v_operational_fit::numeric*0.18+
      v_route_quality::numeric*0.24+
      v_route_confidence::numeric*0.10+
      v_evidence_quality::numeric*0.10+
      v_commercial_value::numeric*0.08+
      v_urgency::numeric*0.06
    )::integer));
    ...
    if v_route_quality<50 then v_limitations:=v_limitations||jsonb_build_array('No strong supported access route is currently available.'); end if;
    ...
    if v_contact.id is null then
      ...
      v_action:='Continue route research before engagement.';
      v_needs_contact:=v_needs_contact+1;
    elsif v_route_quality<35 then
      v_status:='NEEDS_CONTACT';
      v_action:='Continue route research before engagement.';
      v_needs_contact:=v_needs_contact+1;
    ...
    v_explanation:=jsonb_build_object(
      'version','opportunity-score/v2-route-quality',
      'weights',jsonb_build_object(
        'companyFit',24,'operationalFit',18,'routeQuality',24,'routeConfidence',10,
        'evidenceQuality',10,'commercialValue',8,'urgency',6
      ),
      'components',jsonb_build_object(
        'companyFit',v_company_fit,'operationalFit',v_operational_fit,
        'routeQuality',v_route_quality,'routeConfidence',v_route_confidence,
        'buyingAuthority',v_buying_authority,'routeAccessibility',v_contactability,
      ...
      'recommendedEntryStrategy',v_entry_strategy,
    ...
    update public.opportunities set
      ...
      route_quality=v_route_quality,
      route_confidence=v_route_confidence,
      ...
      recommended_entry_strategy=v_entry_strategy,
      score_explanation_json=v_explanation,
      scoring_version='opportunity-score/v2-route-quality',
      scored_at=now(),
      updated_at=now()
    where id=v_opp.id;
    v_scored:=v_scored+1;
    ...
    if v_previous_score is distinct from v_score then
      insert into public.opportunity_history(
        ...
        v_opp.rank,v_opp.rank,jsonb_build_object(
          ...
          'scoringVersion','opportunity-score/v2-route-quality','routeQuality',v_route_quality,'routeConfidence',v_route_confidence,'schedulerRunId',p_scheduler_run_id
        )
      );
    end if;
      ...
      select id,organisation_id,campaign_id,status,rank as old_rank,
        row_number() over(
          ...
          order by
            ...
            opportunity_score desc nulls last,route_quality desc nulls last,updated_at desc,id
        )::integer as new_rank
  ...
  loop
    ...
    insert into public.opportunity_history(
      ...
      jsonb_build_object('rankingMode','ROUTE_AWARE_OPPORTUNITY_SCORE','schedulerRunId',p_scheduler_run_id)
    );
